...
resource "bigip_ltm_virtual_server" "vs_tf" {
  ...
  profiles                   = [bigip_ltm_profile_tcp.profile_tf[count.index].name]
  ...
  depends_on = [
    ...
    bigip_ltm_profile_tcp.profile_tf
  ]
}

# profile 
resource "bigip_ltm_profile_tcp" "profile_tf" {
  count         = length(var.app_port)
  name          = "/${var.app_partition}/tcp_${var.app_name}_${var.app_port[count.index]}"
...
resource "bigip_ltm_monitor" "monitor_tf" {
  count  = length(var.app_port)
  name   = "/${var.app_partition}/monitor_${var.app_name}_${var.app_port[count.index]}_tcp"
  parent = "/Common/tcp"
}
